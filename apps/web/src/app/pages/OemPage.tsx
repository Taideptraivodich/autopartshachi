...
const OemPage: React.FC = () => {
  ...
  const hasResults =
    hasSearched && !loading && !error && (results?.length ?? 0) > 0;
        ...
        <div className={styles.page}>
          <div className={styles.hero}>
            ...
            <p className={styles.subtitle}>
              Nhập mã OEM của hãng xe để tìm phụ tùng phù hợp. Hỗ trợ cả định
              dạng có gạch ngang (ví dụ: <code>04465-BZ160</code>) và liền (ví
              dụ: <code>04465BZ160</code>).
            </p>
          </div>
          ...
          >
            ...
            <p className={styles.hint}>
              Mã OEM thường in trên bao bì phụ tùng gốc hoặc trong tài liệu kỹ
              thuật của xe.
            </p>
          </form>
          ...
          {isEmpty && (
            <div className={styles.emptyState}>
              ...
              <p className={styles.stateDesc}>
                Không có phụ tùng nào khớp với mã OEM{" "}
                <strong>"{queriedCode}"</strong>.
              </p>
              <ul className={styles.tips}>
                <li>Kiểm tra lại mã OEM trên bao bì hoặc tài liệu xe.</li>
                <li>
                  Thử bỏ gạch ngang, ví dụ <code>04465BZ160</code> thay vì{" "}
                  <code>04465-BZ160</code>.
                </li>
                <li>
                  Hoặc{" "}
                  <Link
                    to={`/tim-kiem?q=${encodeURIComponent(queriedCode ?? "")}`}
            ...
            <div className={styles.results}>
              <div className={styles.resultsHeader}>
                <h2 className={styles.resultsTitle}>
                  Kết quả cho mã{" "}
                  <span className={styles.codeHighlight}>"{queriedCode}"</span>
                </h2>

                <span className={styles.resultsCount}>
                  {results!.length} phụ tùng
                </span>
              </div>
              ...
              <div className={styles.resultList}>
                {results!.map((item) => (
                  <div
                    key={`${item.productId}-${item.matchedOemCode}`}
                    className={styles.resultItem}
                  >
                    <div className={styles.oemMeta}>
                      <span className={styles.oemCode}>
                        {item.matchedOemCode}
                      </span>
                    </div>
