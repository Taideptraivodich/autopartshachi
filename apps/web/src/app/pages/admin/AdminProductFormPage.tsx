...
const emptyCompatRow = (): CompatibilityRow => ({
  key: newRowKey(), vehicleBrandId: "", models: [], vehicleModelId: "", yearStart: "", yearStartMode: "year",
  yearEnd: "", untilNow: false, allYears: false, installationPosition: "chung",
});
...
const AdminProductFormPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [b, c, vb] = await Promise.all([fetchAllBrands(), fetchAllCategories(), fetchAllVehicleBrands()]);
        setBrands(b.data); setCategories(c.data); setVehicleBrands(vb.data);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setName(data.name); setSku(data.sku); setProductBrandId(String(data.productBrandId)); setStatus(data.status);
        setDescription(data.description ?? ""); setSpecification(data.specification ?? ""); setCategoryIds(data.categoryIds);
        setImages(data.images); setOemCodesText(data.oemCodes.join("\n"));
        setCompatRows(data.compatibility.map((entry) => {
          const allYears = entry.yearStart == null && entry.yearEnd == null;
          return {
            key: newRowKey(), vehicleBrandId: "", models: [], vehicleModelId: String(entry.vehicleModelId),
            yearStart: entry.yearStart == null ? "" : String(entry.yearStart),
            yearStartMode: entry.yearStart == null ? "before" : "year",
            yearEnd: entry.yearEnd == null ? "" : String(entry.yearEnd), untilNow: entry.yearEnd == null, allYears,
  ...
  const handleQuickCreateLogoSelect = async (file: File) => {
    setQuickCreateUploading(true); setError(null);
    try { setQuickCreateLogoUrl(await uploadImage(file)); }
    catch (err) { setError(`Upload logo thất bại: ${(err as Error).message}`); }
    finally { setQuickCreateUploading(false); }
  };
  const handleQuickCreate = async () => {
    const trimmed = quickCreateName.trim(); if (!trimmed || !quickCreateKind || quickCreateUploading) return;
    setQuickCreateSaving(true); setError(null);
    try {
      if (quickCreateKind === "brand") {
        const created = await createAdminBrand(trimmed, true, quickCreateLogoUrl);
        setBrands((prev) => [...prev, created].sort((a, b) => a.name.localeCompare(b.name))); setProductBrandId(String(created.id));
      } else {
        const created = await createAdminCategory({ name: trimmed, parentCategoryId: null, isActive: true });
        const createdCategory: CategoryListItem = { id: created.id, name: created.name, slug: created.slug, parentCategoryId: created.parentCategoryId, displayOrder: 0 };
  ...
  const handleFileSelect = async (idx: number, file: File) => {
    ...
    try { updateImage(idx, { imageUrl: `http://localhost:3001${await uploadImage(file)}` }); }
    catch (e) { setError(`Upload ảnh thất bại: ${(e as Error).message}`); }
  ...
  const onCompatBrandChange = useCallback(async (key: string, brandId: string) => {
    setCompatRows((prev) => prev.map((r) => r.key === key ? { ...r, vehicleBrandId: brandId, models: [], vehicleModelId: "", yearStart: "", yearStartMode: "year", yearEnd: "", untilNow: false, allYears: false } : r));
    const brand = vehicleBrands.find((b) => String(b.id) === brandId); if (!brand) return;
    try { const { data } = await fetchVehicleBrandBySlug(brand.slug); setCompatRows((prev) => prev.map((r) => r.key === key ? { ...r, models: data.models } : r)); }
    catch (err) { setError((err as Error).message); }
  }, [vehicleBrands]);
  ...
  const handleStartModeChange = (key: string, value: string) => {
    if (value === BEFORE_YEAR) updateCompatRow(key, { yearStartMode: "before", yearStart: "", allYears: false });
    else updateCompatRow(key, { yearStartMode: "year", yearStart: value, allYears: false });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !sku.trim() || !productBrandId) { setError("Vui lòng nhập đủ Tên sản phẩm, SKU và Thương hiệu."); return; }
    const oemCodes = oemCodesText.split(/[\n,]/).map((s) => s.trim()).filter(Boolean);
    for (const r of compatRows) {
      if (!r.vehicleModelId) continue;
      if (!r.allYears && r.yearStartMode === "year" && !r.yearStart) { setError("Mỗi xe tương thích cần có Từ năm hoặc chọn Từ trước."); return; }
      if (!r.allYears && !r.yearEnd && r.yearStartMode === "before") { setError("Khi chọn Từ trước, cần nhập Đến năm."); return; }
      if (!r.allYears && !r.untilNow && r.yearStartMode === "year" && r.yearStart && r.yearEnd && Number(r.yearEnd) < Number(r.yearStart)) { setError("Đến năm không được nhỏ hơn Từ năm."); return; }
    }
    const compatibility: AdminProductCompatibilityPayload[] = compatRows.filter((r) => r.vehicleModelId).map((r) => ({
      vehicleModelId: Number(r.vehicleModelId),
      yearStart: r.allYears || r.yearStartMode === "before" ? null : Number(r.yearStart),
      yearEnd: r.allYears || r.untilNow ? null : (r.yearEnd ? Number(r.yearEnd) : null),
    ...
    setSaving(true);
    try { if (isEdit) await updateProduct(Number(id), payload); else await createProduct(payload); navigate("/admin/san-pham"); }
    catch (err) { setError((err as Error).message); }
  ...
  return (
    <div style={styles.page}>
      <header style={styles.header}><span style={styles.logo}>Hachi Admin</span><Button variant="ghost" size="sm" onClick={logout}>Đăng xuất</Button></header>
          ...
          <form onSubmit={handleSubmit}>
            <section style={styles.section}>
              ...
              <div style={styles.actionField}>
                <Select label="Thương hiệu *" value={productBrandId} onChange={(e) => setProductBrandId(e.target.value)} placeholder="— Chọn thương hiệu —" options={brands.map((b) => ({ value: String(b.id), label: b.name }))} required />
                <Button type="button" variant="secondary" size="sm" onClick={() => openQuickCreate("brand")}>+ Thương hiệu mới</Button>
              </div>
            ...
            <section style={styles.section}>
              <div style={styles.sectionTitleRow}><h3 style={{ margin: 0 }}>Danh mục</h3><Button type="button" variant="secondary" size="sm" onClick={() => openQuickCreate("category")}>+ Danh mục mới</Button></div>
              <div style={styles.checkGrid}>{categories.map((c) => <Checkbox key={c.id} label={c.name} checked={categoryIds.includes(c.id)} onChange={() => toggleCategory(c.id)} />)}</div>
            </section>
            ...
            <section style={styles.section}>
              ...
              {images.map((img, idx) => (
                <div key={idx} style={styles.imageRow}>
                  <div style={{ flex: 1 }}><div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}><Input placeholder="Dán URL ảnh hoặc tải lên từ máy →" value={img.imageUrl} onChange={(e) => updateImage(idx, { imageUrl: e.target.value })} style={{ flex: 1 }} /><label style={styles.uploadBtn}>{uploadingIdx === idx ? "Đang tải" : "Tải ảnh"}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" style={{ display: "none" }} disabled={uploadingIdx !== null} onChange={(e) => { const file = e.target.files?.[0]; if (file) handleFileSelect(idx, file); e.target.value = ""; }} /></label></div>{img.imageUrl && <img src={resolveImageUrl(img.imageUrl) ?? img.imageUrl} alt="preview" style={styles.imgPreview} />}</div>
                  <Checkbox label="Ảnh đại diện" checked={!!img.isThumbnail} onChange={() => setThumbnail(idx)} /><Button type="button" variant="danger" size="sm" onClick={() => removeImage(idx)}>Xóa</Button>
                </div>
              ))}
            ...
            <section style={styles.section}>
              <div style={styles.compatHeader}><div><h3 style={{ marginBottom: 4 }}>Xe tương thích</h3><p style={styles.compatHint}>Chọn Hãng → Dòng → khung năm áp dụng cho chính sản phẩm này.</p></div></div>
              {compatRows.map((row) => (
                <div key={row.key} style={styles.compatRow}>
                  ...
                  <div style={styles.yearField}>
                    ...
                    <select value={row.allYears ? BEFORE_YEAR : row.yearStartMode === "before" ? BEFORE_YEAR : row.yearStart} onChange={(e) => handleStartModeChange(row.key, e.target.value)} disabled={row.allYears} style={styles.yearSelect}>
                      <option value="">— Chọn năm —</option><option value={BEFORE_YEAR}>Từ trước</option>{YEAR_OPTIONS.map((y) => <option key={y} value={y}>{y}</option>)}
                    </select>
                  </div>
                  <div style={styles.yearField}>
                    <label style={styles.yearLabel}>Đến năm</label>
                    <select value={row.untilNow ? "" : row.yearEnd} onChange={(e) => updateCompatRow(row.key, { yearEnd: e.target.value })} disabled={row.untilNow || row.allYears || row.yearStartMode === "before"} style={styles.yearSelect}>
                      <option value="">— Chọn năm —</option>{YEAR_OPTIONS.map((y) => <option key={y} value={y}>{y}</option>)}
                    </select>
                    <label style={styles.untilNowLabel}><input type="checkbox" checked={row.untilNow} disabled={row.allYears || row.yearStartMode === "before"} onChange={(e) => updateCompatRow(row.key, { untilNow: e.target.checked, yearEnd: "" })} />Đến nay</label>
                  </div>
                  ...
                  <Select label="Vị trí lắp" value={row.installationPosition} onChange={(e) => updateCompatRow(row.key, { installationPosition: e.target.value })} options={POSITION_OPTIONS} />
