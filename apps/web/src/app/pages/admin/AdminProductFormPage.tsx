...
const emptyCompatRow = (): CompatibilityRow => ({
  key: newRowKey(), vehicleBrandId: "", models: [], vehicleModelId: "",
  yearStart: "", yearStartMode: "year", yearEnd: "", yearEndMode: "year", installationPosition: "chung",
});
...
const AdminProductFormPage: React.FC = () => {
    ...
    (async () => {
      ...
      try {
        ...
        const loadedRows = data.compatibility.map((entry) => ({
          ...
          yearEndMode: entry.yearEnd == null ? "now" : "year",
  ...
  const onCompatBrandChange = useCallback(async (key: string, brandId: string) => {
    setCompatRows((prev) => prev.map((r) => r.key === key ? { ...r, vehicleBrandId: brandId, models: [], vehicleModelId: "", yearStart: "", yearStartMode: "year", yearEnd: "", yearEndMode: "year" } : r));
  ...
  const onCompatModelChange = useCallback((key: string, modelId: string) => setCompatRows((prev) => prev.map((r) => r.key === key ? { ...r, vehicleModelId: modelId, yearStart: "", yearStartMode: "year", yearEnd: "", yearEndMode: "year" } : r)), []);
  ...
  const handleStartModeChange = (key: string, value: string) => {
    if (value === BEFORE_YEAR) updateCompatRow(key, { yearStartMode: "before", yearStart: "" });
    else updateCompatRow(key, { yearStartMode: "year", yearStart: value });
  };
  const handleEndModeChange = (key: string, value: string) => {
    if (value === UNTIL_NOW) updateCompatRow(key, { yearEndMode: "now", yearEnd: "" });
    else updateCompatRow(key, { yearEndMode: "year", yearEnd: value });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const r of compatRows) {
      ...
      if (r.yearStartMode === "year" && !r.yearStart) { setError("Mỗi xe tương thích cần có Từ năm hoặc chọn Từ trước."); return; }
      if (r.yearEndMode === "year" && !r.yearEnd) { setError("Mỗi xe tương thích cần có Đến năm hoặc chọn Đến nay."); return; }
      if (r.yearStartMode === "year" && r.yearEndMode === "year" && r.yearStart && r.yearEnd && Number(r.yearEnd) < Number(r.yearStart)) { setError("Đến năm không được nhỏ hơn Từ năm."); return; }
    }
    const compatibility: AdminProductCompatibilityPayload[] = compatRows.filter((r) => r.vehicleModelId).map((r) => ({
      ...
      yearStart: r.yearStartMode === "before" ? null : Number(r.yearStart),
      yearEnd: r.yearEndMode === "now" ? null : Number(r.yearEnd),
      installationPosition: r.installationPosition,
    }));
    const payload = {
      productBrandId: Number(productBrandId), sku: sku.trim(), name: name.trim(),
      description: description.trim() || undefined, specification: specification.trim() || undefined,
      status: status as "con_hang" | "het_hang" | "ngung_kinh_doanh", categoryIds,
      images: images.filter((img) => img.imageUrl.trim()), oemCodes, compatibility,
    };
    setSaving(true);
  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        ...
        {loading ? <p>Đang tải...</p> : <form onSubmit={handleSubmit}>
          ...
          <section style={styles.section}><div style={styles.compatHeader}><div><h3 style={{ marginBottom: 4 }}>Xe tương thích</h3><p style={styles.compatHint}>Chọn Hãng → Dòng → khung năm áp dụng cho chính sản phẩm này.</p></div></div>{compatRows.map((row) => <div key={row.key} style={styles.compatRow}>
            ...
            <div style={styles.yearField}><label style={styles.yearLabel}>Từ năm</label><select value={row.yearStartMode === "before" ? BEFORE_YEAR : row.yearStart} onChange={(e) => handleStartModeChange(row.key, e.target.value)} style={styles.yearSelect}><option value="">— Chọn năm —</option><option value={BEFORE_YEAR}>Từ trước</option>{YEAR_OPTIONS.map((y) => <option key={y} value={y}>{y}</option>)}</select></div>
            <div style={styles.yearField}><label style={styles.yearLabel}>Đến năm</label><select value={row.yearEndMode === "now" ? UNTIL_NOW : row.yearEnd} onChange={(e) => handleEndModeChange(row.key, e.target.value)} style={styles.yearSelect}><option value="">— Chọn năm —</option><option value={UNTIL_NOW}>Đến nay</option>{YEAR_OPTIONS.map((y) => <option key={y} value={y}>{y}</option>)}</select></div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  uploadBtn: { display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 72, height: 36, borderRadius: 6, border: "1px solid #d1d5db", background: "#f9fafb", cursor: "pointer", fontSize: "0.8rem", flexShrink: 0, padding: "0 10px" },
  imgPreview: { display: "block", marginTop: "0.5rem", maxHeight: 80, maxWidth: 120, borderRadius: 4, border: "1px solid #e5e7eb", objectFit: "cover" },
  compatHeader: { marginBottom: "0.75rem" },
  compatHint: { margin: 0, color: "#667085", fontSize: "0.875rem" },
  compatRow: { display: "grid", gridTemplateColumns: "1.05fr 1.2fr 0.9fr 0.9fr 1fr auto", gap: "0.75rem", alignItems: "start", marginBottom: "0.75rem", padding: "0.9rem 0", borderBottom: "1px dashed #e5e7eb" },
  yearField: { display: "flex", flexDirection: "column", gap: 6 },
  yearLabel: { fontSize: 13, fontWeight: 500, color: "#344054" },
  yearSelect: { width: "100%", minHeight: 40, boxSizing: "border-box", padding: "8px 10px", border: "1px solid #d0d5dd", borderRadius: 6, fontSize: 14, background: "#fff" },
  ...
  quickLogoPreview: { width: 56, height: 56, objectFit: "contain", border: "1px solid #e5e7eb", borderRadius: 6, background: "#f9fafb" },
  quickLogoRemove: { fontSize: "0.78rem", color: "#b91c1c", background: "none", border: "none", cursor: "pointer", padding: 0 },
  quickLogoUploadBtn: { display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 130, height: 36, borderRadius: 6, border: "1px solid #d1d5db", background: "#fff", cursor: "pointer", fontSize: "0.82rem", color: "#374151", padding: "0 12px" },
