...
interface OEMBlockProps { codes: OemCode[] }
...
const OEMBlock: React.FC<OEMBlockProps> = ({ codes }) => (
  <div className={styles.block}>
    <div className={styles.header}>
      <span className={styles.headerMark} aria-hidden="true">OEM</span>
    ...
    {codes.length === 0 ? (
      ...
      <table className={styles.table}>
        <thead><tr><th>Mã OEM</th><th>Hãng phát hành</th><th>Trạng thái</th><th>Độ khớp</th></tr></thead>
        <tbody>
          {codes.map((oem) => (
            <tr key={oem.id}>
              <td><span className={styles.code}>{oem.code}</span></td>
              <td>{oem.issuingBrand ?? '—'}</td>
              <td><span className={`${styles.statusBadge} ${styles[`status--${oem.status}`]}`}>{STATUS_LABEL[oem.status] ?? oem.status}</span></td>
              <td><span className={styles.confidence}>{CONFIDENCE_LABEL[oem.matchConfidence] ?? oem.matchConfidence}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
