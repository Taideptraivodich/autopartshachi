...
function mapDetailToResult(p: ProductDetail): ProductDetailResult {
  ...
  return {
    ...
    // Lưu ý: frontend dùng `code` (không phải `oemNumber`).
    // `issuingBrand` cần tên hãng xe (string), nhưng repo hiện chỉ trả
    // `issuingVehicleBrandId` — để null ở MVP, join tên hãng sau.
