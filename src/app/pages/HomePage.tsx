...
const FeaturedProductCard: React.FC<{ product: ProductListItem }> = ({
  product,
}) => (
  <Link to={`/san-pham/${product.slug}`} className={styles.productCard}>
    ...
    <div className={styles.productBody}>
      ...
      {product.status === "con_hang" && (
        <span className={styles.productStatus}>Còn hàng</span>
      )}
    </div>
  </Link>
);
      ...
      <div className="container">
        ...
        {loading ? (
          ...
          >
            ...
            <Link to="/san-pham" style={{ color: "var(--color-text-link)" }}>
              Xem tất cả →
            </Link>
          </p>
        ) : (
      ...
      <div className="container">
        ...
        {loading ? (
          <div className={styles.brandGrid}>
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className={styles.skeletonBrand} />
            ))}
          </div>
        ) : brands.length === 0 ? (
          <p
          ...
          >
            ...
            <div className={styles.searchTags}>
              ...
              {[
                ...
                <Link key={tag} to="/san-pham" className={styles.tag}>
                  {tag}
                </Link>
              ))}
            </div>
            ...
            {[
              {
                ...
                desc: "100% phụ tùng có xuất xứ rõ ràng, tem nhãn đầy đủ.",
              },
              {
                icon: "🔍",
                title: "Tra cứu mã OEM",
                desc: "Hỗ trợ tra cứu theo số khung, số máy và mã OEM gốc.",
              },
              {
                icon: "🚚",
                title: "Giao nhanh toàn quốc",
                desc: "Giao hàng trong 24–48 giờ tại TP.HCM và các tỉnh thành.",
              },
              {
                icon: "🛡",
                title: "Bảo hành chính sách",
                desc: "Bảo hành theo tiêu chuẩn nhà sản xuất, đổi trả dễ dàng.",
              },
            ].map((usp) => (
